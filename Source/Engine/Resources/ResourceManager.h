...
namespace neu {
    ...
    public:
		...
		template<typename T = Resource>
			requires std::derived_from<T, Resource>
        bool AddResource(const std::string& name, res_t<T> resource);
    private:
        /// <summary>
	...
	template<typename T>
		requires std::derived_from<T, Resource>
    inline bool ResourceManager::AddResource(const std::string& name, res_t<T> resource)
    {
